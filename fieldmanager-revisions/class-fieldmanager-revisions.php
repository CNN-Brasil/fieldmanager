...
class Fieldmanager_Revisions {
	...
	public function save_revision_post_meta( $post_id ) {
		...
		if ( $parent_id && get_post_type( $parent_id ) === $this->post_type ) {
			...
			foreach ( $this->meta_fields as $key => $label ) {
				$label;
				if ( isset( $_POST[ $key ] ) && isset( $_POST['fieldmanager-block_list-nonce'] ) && wp_verify_nonce( sanitize_text_field( $_POST['fieldmanager-block_list-nonce'] ), 'fieldmanager-save-block_list' ) ) {
					//scape in fieldmanager-revisions.php:L60
					$value = _fieldmanager_sanitize_deep( $_POST[ $key ] ); // phpcs:ignore
					$meta[ $key ] = $value;
					update_metadata( 'post', $post_id, $key, $value );
				} elseif ( get_post_meta( $parent_id, $key, true ) ) {
	...
	public function action__save_post( $post_id ) {
		...
		foreach ( $this->meta_fields as $key => $label ) {
			...
			if ( isset( $_POST[ $key ] ) && isset( $_POST['fieldmanager-block_list-nonce'] ) && wp_verify_nonce( sanitize_text_field( $_POST['fieldmanager-block_list-nonce'] ), 'fieldmanager-save-block_list' ) ) {
					//scape in fieldmanager-revisions.php:L60
				$meta[ $key ] = _fieldmanager_sanitize_deep( $_POST[ $key ] ); // phpcs:ignore
